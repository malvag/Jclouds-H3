...
public final class H3BlobStore implements BlobStore {
	...
	@Inject
	H3BlobStore(BlobStoreContext context,
				BlobUtils blobUtils,
	...
	private SortedSet<StorageMetadata> extractCommonPrefixes(SortedSet<StorageMetadata> contents, String delimiter,
		...
		SortedSet<String> commonPrefixes = newTreeSet(
				transform(contents, new H3BlobStore.CommonPrefixes(prefix, delimiter)));
		commonPrefixes.remove(H3BlobStore.CommonPrefixes.NO_PREFIX);

		contents = newTreeSet(filter(contents, new H3BlobStore.DelimiterFilter(prefix, delimiter)));
